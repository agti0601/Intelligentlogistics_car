...
from tft.sysfont import sysfont
...
class TFT(object) :
  ...
  def __init__( self, spi, dc, rst, cs,blk=None) :
    ...
    if blk is not None:
        self._light=machine.PWM(machine.Pin(blk),freq=1000,duty=500)
    else:
        self._light=None
...
# light 0-100
  def light(self,level=None):
      if self._light is None:
          return None
      if level is None:
          return int(self._light.duty()/10)
    
      if level>100:
          level=100
      elif level<0:
          level=0
      self._light.duty(level*10)
